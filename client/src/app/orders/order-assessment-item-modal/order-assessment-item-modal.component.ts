...
export class OrderAssessmentItemModalComponent implements OnInit {
  ...
  CreateAndInitializeFormArray(item: IOrderAssessmentItem) {
    ...
    this.form = this.fb.group({
        ...
        professionGroup: [item.professionGroup ?? ""],
  ...
  closeSection() {
    //this.copyAssessmentQs=false;
  }

  getProfessionGroup() {
      var professionId = this.form.get('professionId')?.value;
      var orderItemAssessmentId = this.form.get('id')?.value;

      this.service.getAndSetProfessionGroupFromProfId(professionId, orderItemAssessmentId).subscribe({
        next: (response: ISingleStringDto) => {
          if(response.stringValue==='') {
            this.toastr.warning('No Order Assessment Items exist matching the desired profession Group ' + this.form.get('professionGroup')?.value, 'No matching assessments exist')
          } else {
            this.orderAssessmentItem!.professionGroup = response.stringValue;
          }
          
        }
        , error: (err: any) => console.log(err.error?.details, 'Error encountered in getProfessionGroup')
      })
  }
  ...
  copyProfGroup() {
    ...
    this.copyAssessmentQs = !this.copyAssessmentQs;   
    if(this.copyAssessmentQs)this.getProfessionGroup();

    var headersDto: IOrderAssessmentItemHeaderDto[]=[];
    if(this.copyAssessmentQs) {   
      var profGroup = this.form.get('professionGroup')?.value;
      this.service.getOrderAssessmentItemHeaderFromProfessionGroup(profGroup).subscribe({   //this is repeated in orderassessmentitemhead.component
        /* **TODO** switchMap this with getProfessionGroup */
        next: (response: IOrderAssessmentItemHeaderDto[]) => {
          console.log(response);
          if(response.length===0) {
            this.toastr.warning('this profession Group does not have any corresponding Order Assessment Items defined', 'not found');
            this.copyAssessmentQs=false;    //close the DOM
            return;
          }
        }
      })
  ...
  assessmentQsEmitted(event: IOrderAssessmentItemQ[]) {
    this.orderAssessmentItemQs.clear();
    this.newOrderAssessmentItemQFromStddQ(event);
    this.calcualteTotals();
    this.copyAssessmentQs=false;
  }
}
