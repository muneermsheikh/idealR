...
//checklist
  ...
  chooseSelectedOrderItem() {
      ...
      this.requireInternalReview = this.orderItemSelected.requireInternalReview==="1";
      ...
      this.service.getCandidateChecklistAndAssessment(this.cvBrief!.id, this.orderItemSelectedId).subscribe({
        next: (response: any) => {
            ...
            if (this.cvAssessment) {
              ...
              if(this.requireInternalReview) this.toastr.warning('the candidate has not been assessed for the category selected.');
            }
  ...
  updateAssessment() {
    ...
    if(this.cvAssessment) {
      ...
      if(this.cvAssessment && this.cvAssessment.candidateAssessmentItems === null) {
        var newCVAssessment: ICandidateAssessment ={
          assessResult:this.form.get('assessResult')!.value,
          assessedByEmployeeName:this.user!.userName,
          assessedOn: new Date(),
          candidateId: this.cvAssessment.candidateId,
          checklistHRId: this.cvAssessment.checklistHRId,
          id: this.cvAssessment.id,
          orderItemId: this.cvAssessment.orderItemId,
          requireInternalReview: this.cvAssessment.requireInternalReview,
          customerName: this.cvAssessment.customerName || this.orderItemSelected!.customerName,
          categoryRefAndName: this.cvAssessment.categoryRefAndName || this.orderItemSelected!.categoryRefAndName,
          orderDate: this.cvAssessment.orderDate,
          taskIdDocControllerAdmin: this.cvAssessment.taskIdDocControllerAdmin,
          candidateAssessmentItems: this.cvAssessment.candidateAssessmentItems,
          remarks: this.cvAssessment.remarks
        };
          ...
          apiCallInternal.pipe(
            ...
            tap(res => {
              ...
              if(res==='') {
                ...
                this.service.getCandidateChecklistAndAssessment(this.cvBrief!.id, this.orderItemSelectedId)
                  .subscribe({
  ...
  openChecklistModal() {
    ...
    if(this.checklist ===null) {
      ...
      this.bsModalRef.content.updateObj.subscribe((response: IChecklistAndCandidateAssessmentDto) => {
          if(response === null) {
            this.toastr.warning("Failed to update the Checklist object -" + response);
          } else {
            this.toastr.success('Updated the checklist object');
            this.checklist!.checklistedOk=true;
  ...
  initializeWithArray(assess: ICandidateAssessment) {
      ...
      console.log('initializedWithArray');
      this.form = this.fb.group({
        ...
        assessedByEmployeeName: assess.assessedByEmployeeName || this.user?.userName,
        categoryRefAndName : assess.categoryRefAndName, customerName: assess.customerName,
        checklistHRId: assess.checklistHRId, 
  ...
  initializeWithoutArray(assess: ICandidateAssessment) {
    ...
    this.form = this.fb.group({
      id:assess.id, orderItemId: assess.orderItemId, candidateId: assess.candidateId, 
      assessedOn: assess.assessedOn, assessResult: assess.assessResult, remarks: assess.remarks,
      requireInternalReview: assess.requireInternalReview, assessedByEmployeeName: assess.assessedByEmployeeName,
      categoryRefAndName : assess.categoryRefAndName
    });
  ...
  calculatePercentage() {
    ...
    this.percentage = (this.totalGained===undefined || this.totalPoints === undefined) 
  ...
  calculateGrade() {
    ...
    this.cvAssessment!.assessResult! = grade;
    this.form.get('assessResult')?.setValue(grade);
    return grade;
  }
  ...
  displayAssessment(candidateAssessmentId: number) {
    this.candidateAssessmentService.getCandidateAssessmentById(candidateAssessmentId).subscribe({
      next: (response: ICandidateAssessment) => {
        console.log('displayAssessment:', response);
        if(response) {
          this.cvAssessment = response;
          if(response !==null && response.candidateAssessmentItems === null) {
            this.initializeWithoutArray(response)
          } else if(response !== null && response.candidateAssessmentItems !== null) {
            this.initializeWithArray(response)
          }
